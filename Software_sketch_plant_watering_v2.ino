...
void loop() {
  ...
  if ((timeNow - lastDebounceTime) >= debounceInterval) {
    if (buttonReading != buttonState) {
      ...
      if (buttonState == LOW) { //Buttonstate is low when button is pressed
        int newMode = (deviceMode == 0) ? 1 : 0; //Change device mode when button is clicked
        snprintf (msg, MSG_BUFFER_SIZE, "%ld", newMode);
        client.publish("infob3it/053/mode", msg); //Publish new device mode
      }
    }
  }
  ...
  if (waterMode == 1) { //While watering, continuously check soil moisture value
    readSoilMoisture();
  }
...
//  readSoilMoisture();
  ...
  display.println(F("Moisture Level: "));o
